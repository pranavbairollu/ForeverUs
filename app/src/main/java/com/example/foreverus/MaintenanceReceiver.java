...
public class MaintenanceReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        Executors.newSingleThreadExecutor().execute(() -> {
            try {
                // Fetch all dates (Logic is self-contained in Repo/DAO usually, but here we can
                // just query DAO directly)
                ...
                // This automatically handles "Year Rollover" because ".scheduleNotifications"
                // calls ".getNextOccurrence()"
                // which uses "System.currentTimeMillis()".
                // If "Event Date" was Nov 6, and Today is Nov 7, getNextOccurrence now returns
                // Nov 6 NEXT YEAR.
                // NotificationScheduler.scheduleNotifications(context,
                // dao.getAllSpecialDatesRaw()); // Legacy removed
