...
public class SpecialDateRepository {
    ...
    private void synchronizeSpecialDates(String relationshipId) {
        ...
        // 0. Attempt to sync any local-only items first (Recovery) - Run ONCE on init,
        // not every snapshot
    ...
    private void uploadUnsyncedSpecialDates(String relationshipId) {
        ...
        for (SpecialDate date : unsynced) {
            // FINAL SAFETY CHECK: Re-fetch to ensure Listener didn't just sync it (Fuzzy
            // Match) a millisecond ago.
    ...
    public void addSpecialDate(String relationshipId, SpecialDate specialDate) {
        executorService.execute(() -> {
            ...
            Log.d(TAG, "Attempting to add Special Date to Firestore: Path=relationships/" + relationshipId
                    + "/specialDates");
            ...
            firestore.collection("relationships").document(relationshipId).collection("specialDates")
                    ...
                    .addOnSuccessListener(documentReference -> {
                        ...
                        executorService.execute(() -> {
                            ...
                            // If it has a DocumentID, we assume the Listener handled it or we handle it
                            // now.
                            ...
                            try {
                                // Fetch the specific record we inserted purely by reference if possible, or by
                                // ID
                                ...
                                if (specialDate.getId() != 0) {
                                    ...
                                    if (latestLocal == null) {
                                        ...
                                        Log.d(TAG,
                                                "Zombie Avoided: User deleted date during upload. Deleting remote copy.");
                                        // Cleanup remote
                                        documentReference.delete();
                                        return;
                                    }
